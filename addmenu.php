...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($kategori_id) || $harga <= 0) {
      if (!$db_error)
        $db_error = "Nama, Kategori, dan Harga (lebih dari 0) wajib diisi.";
    } else {
      // Insert ke tabel menu
      $stmtInsert = $pdo->prepare("
                INSERT INTO menu (kategori_id, shop_id, nama, deskripsi, harga, gambar_url, stok) 
                VALUES (:kategori_id, :shop_id, :nama, :deskripsi, :harga, :gambar_url, :stok)
            ");
