<?php
// ============================================================
// app.php — Halaman Pemesanan WarungKu (dengan Database)
// ============================================================
...
try {
  ...
  // Cek masa aktif langganan toko
  $stmtSub = $pdo->prepare("SELECT subscription_ends_at FROM shops WHERE id = :shop_id");
  $stmtSub->execute(['shop_id' => $shop_id]);
  $shop = $stmtSub->fetch();
  
  if ($shop && $shop['subscription_ends_at'] && strtotime($shop['subscription_ends_at'] . ' 23:59:59') < time()) {
      echo '<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Langganan Habis</title><style>body{background:#0f0e0b;color:#f0e8d5;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;text-align:center;} .box{background:#1a1814;padding:40px;border-radius:16px;border:1px solid rgba(224,82,82,0.3);max-width:400px;} h1{color:#e05252;} a{display:inline-block;margin-top:20px;padding:10px 20px;background:#d4a853;color:#0f0e0b;text-decoration:none;border-radius:8px;font-weight:bold;}</style></head><body><div class="box"><h1>Masa Aktif Habis</h1><p>Masa berlangganan toko Anda telah berakhir pada <b>'.date('d M Y', strtotime($shop['subscription_ends_at'])).'</b>.</p><p>Silakan hubungi Administrator untuk membeli token perpanjangan agar sistem kasir dapat digunakan kembali.</p><a href="logout.php">Keluar</a></div></body></html>';
      exit;
  }
  $stmt = $pdo->prepare("
        SELECT
...
<head>
  ...
  <title>Pesan Menu — WarungKu | Sistem Pemesanan Cafetaria Kampus</title>
  <meta name="description" content="Halaman pemesanan WarungKu. Pilih menu, atur jumlah, dan checkout dalam hitungan detik. Didesain khusus untuk penjual cafetaria kampus.">
  <meta name="keywords" content="pesan makanan cafetaria, sistem kasir kampus, order makanan kantin, WarungKu pemesanan, aplikasi kasir warung">
  <meta name="author" content="WarungKu">
  <meta name="robots" content="index, follow">
  <link rel="canonical" href="https://catat-pesan.web-portofolio.com/index.php">
  <link rel="icon" type="image/png" href="logo.png">

  <!-- Open Graph -->
  <meta property="og:type" content="website">
  <meta property="og:title" content="Pesan Menu — WarungKu">
  <meta property="og:description" content="Pilih menu, atur jumlah, dan checkout dalam hitungan detik. Sistem pemesanan cepat untuk cafetaria kampus.">
  <meta property="og:url" content="https://catat-pesan.web-portofolio.com/index.php">
  ...
  <meta name="twitter:title" content="Pesan Menu — WarungKu">
  <meta name="twitter:description" content="Sistem pemesanan cafetaria kampus. Cepat, akurat, mobile-friendly.">
  ...
  <script type="application/ld+json">
  {
    ...
    "@type": "WebApplication",
    "name": "WarungKu — Halaman Pemesanan",
    "applicationCategory": "BusinessApplication",
    "operatingSystem": "Web",
    "url": "https://catat-pesan.web-portofolio.com/index.php",
    "description": "Halaman pemesanan WarungKu untuk mencatat pesanan cafetaria kampus dengan cepat dan akurat.",
    "image": "https://catat-pesan.web-portofolio.com/logo.png",
    "offers": { "@type": "Offer", "price": "0", "priceCurrency": "IDR" },
    "author": { "@type": "Organization", "name": "WarungKu" }
  }
  </script>
  ...
  <style>
    ...
    * {
    ...
    .r-sum-row.grand span:last-child {
      color: var(--gold);
    }
    ...
    @media (max-width: 900px) {
      ...
      .sidebar.show~.floating-cart-toggle {
        right: -80px;
      }
      ...
      /* Bigger touch targets for + button */
      .qty-btn {
        width: 44px;
        height: 44px;
        font-size: 20px;
...
<body>
  ...
  <?php if ($db_error): ?>
    <div
      style="background:#e05252;color:#fff;padding:12px 32px;font-size:13px;font-weight:600;display:flex;align-items:center;gap:10px;position:sticky;top:0;z-index:200;">
  ...
  <header class="topbar">
    ...
    <div class="topbar-right">
      <a href="addmenu.php"
        style="color:var(--text-dim);font-size:13px;text-decoration:none;display:flex;align-items:center;gap:6px;padding:8px 14px;border:1px solid var(--border);border-radius:8px;transition:all 0.2s;"
      ...
      <button onclick="toggleTheme()" id="themeToggleBtn"
        style="background:transparent;border:1px solid var(--border);border-radius:8px;color:var(--text-dim);cursor:pointer;padding:8px 10px;font-size:14px;transition:all 0.2s;"
        onmouseover="this.style.borderColor='var(--gold)';this.style.color='var(--gold)'"
        onmouseout="this.style.borderColor='var(--border)';this.style.color='var(--text-dim)'">☀️</button>
      <span style="color:var(--text-dim);font-size:13px;" class="hide-mobile">
        Kasir: <strong style="color:var(--cream)"><?= htmlspecialchars($_SESSION['nama_lengkap'] ?? 'Kasir') ?></strong>
        <a href="logout.php" style="color:var(--red);text-decoration:none;margin-left:12px;font-weight:bold;border:1px solid rgba(224,82,82,0.3);padding:4px 8px;border-radius:6px;font-size:11px;">Keluar</a>
      </span>
      ...
      <div class="filter-bar">
        ...
        <?php foreach ($categories as $cat): ?>
          <button class="filter-btn"
            onclick="filterMenu('<?= htmlspecialchars($cat) ?>', this)"><?= htmlspecialchars($cat) ?></button>
        <?php endforeach; ?>
      </div>
      ...
      <div class="menu-grid" id="menuGrid">
        <?php foreach ($menu_items as $item): ?>
          <div class="card" id="card-<?= $item['id'] ?>" data-cat="<?= htmlspecialchars($item['category']) ?>">
            ...
            <img class="card-img" src="<?= htmlspecialchars($item['image']) ?>"
            ...
            <div class="card-body">
              ...
              <div class="card-footer">
                ...
                <div class="qty-ctrl">
                  <button class="qty-btn" onclick="changeQty(<?= $item['id'] ?>, -1)" id="btn-minus-<?= $item['id'] ?>"
                    style="display:none">−</button>
                  <span class="qty-num" id="qty-<?= $item['id'] ?>">0</span>
                  <button class="qty-btn add-btn" onclick="changeQty(<?= $item['id'] ?>, 1)"
                    id="btn-add-<?= $item['id'] ?>">+</button>
                </div>
              </div>
            </div>
          </div>
  ...
  <script>
    ...
    function updateMobileBar() {
      ...
      document.getElementById('mbbCount').textContent = totalItems + ' item';
      document.getElementById('mbbTotal').textContent = fmt(total);
      const mbbBtn = document.getElementById('mbbCheckout');
      mbbBtn.disabled = totalItems === 0;

      bar.style.display = (totalItems > 0 && window.innerWidth <= 900) ? 'flex' : 'none';
    }

    // Patch renderCart to also update mobile bar
    const _origRenderCart = renderCart;
    renderCart = function() { _origRenderCart(); updateMobileBar(); };
  ...
  <div class="mobile-bottom-bar" id="mobileBottomBar">
    <div class="mbb-info">
      <div class="mbb-count" id="mbbCount">0 item</div>
      <div class="mbb-total" id="mbbTotal">Rp 0</div>
    </div>
    <button class="mbb-view-cart" onclick="toggleMobileCart()">🛒 Lihat</button>
    <button class="mbb-checkout" id="mbbCheckout" onclick="doCheckout()" disabled>Checkout →</button>
  </div>
